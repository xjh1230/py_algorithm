...
。在此问题中，当存在平局（即两个或更多个键具有相同使用频率）时，最近最少使用的键将被去除。
        ...
        self.dic_count = {}  # 从放当前引用次数下所有key,按照使用时间依此插入 {1:[key1,key2,key3]}
    ...
    def _set_count(self, key):
        ...
        # 前一个使用次数删除该key
        if count > 1:
            index = self.dic_count[count - 1].index(key)
            self.dic_count[count - 1].pop(index)
        # 后一个使用次数添加该key
        tmp = self.dic_count.get(count, 0)
        if tmp == 0:
            self.dic_count[count] = [key]
        else:
            self.dic_count[count].append(key)
